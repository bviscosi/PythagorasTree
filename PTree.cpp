...
PTree::PTree(double length, int depth, double angle){
    L = length;
    N = depth;
    A = angle;
    ...
    origin = sf::Vector2f(offset - L/2, 4*L);
...
void PTree::pTree(sf::RenderTarget& target, sf::ConvexShape root, double L, int depth, double a, double b) const{
    ...
    pTree(target, left, lengthA, depth-1, a+A, b-A);
    pTree(target, right, lengthB, depth-1,  A-b, b+(90-A));
}

void PTree::draw(sf::RenderTarget& target, sf::RenderStates states) const{
    pTree(target, root, L, N, A, 90-A);
}
